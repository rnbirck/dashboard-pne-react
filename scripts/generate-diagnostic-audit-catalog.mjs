...
const matrixOutputPath = new URL('../src/data/diagnostic/indicatorFinancingMatrix.json', import.meta.url)
...
const allIndicatorIds = indicators.map((indicator) => indicator.indicatorId)
const attendanceIds = [
  'creche', 'pre_escola', 'basico_6_17', 'basico_15_17', 'alfabetizacao',
  'idade_regular_quinto', 'idade_regular_nono', 'idade_regular_medio',
]
const integralIds = ['basico_integral', 'escolas_integral']
const learningIds = [
  'alfabetizacao', 'saeb_matematica_anos_iniciais', 'saeb_matematica_anos_finais',
  'saeb_matematica_ensino_medio', 'saeb_portugues_anos_iniciais',
  'saeb_portugues_anos_finais', 'saeb_portugues_ensino_medio',
]
const teacherIds = ['adequacao_ai', 'adequacao_af', 'adequacao_em', 'pos_graduacao', 'rendimento_magisterio', 'temporarios']
const infrastructureIds = indicators.filter((indicator) => indicator.category === 'infraestrutura').map((indicator) => indicator.indicatorId)

const matrix = {
  indicatorCount: allIndicatorIds.length,
  lastVerifiedAt: '2026-07-19',
  links: [
    { programId: 'fundeb_vaaf', indicatorIds: allIndicatorIds, linkType: 'general_mde', evidence: 'Financia MDE segundo matrículas e ponderações; não há nexo causal direto com um indicador isolado.' },
    { programId: 'fundeb_vaat', indicatorIds: allIndicatorIds, linkType: 'general_mde', evidence: 'Complementação sistêmica condicionada ao VAAT; confirmar benefício nominal do ente.' },
    { programId: 'fundeb_vaar', indicatorIds: [...new Set([...attendanceIds, ...learningIds])], linkType: 'conditioned', evidence: 'A elegibilidade usa condicionalidades e indicadores oficiais; os indicadores do painel não provam habilitação.' },
    { programId: 'salario_educacao_qsem', indicatorIds: allIndicatorIds, linkType: 'general_mde', evidence: 'Quota para programas, projetos e ações da educação básica, sem vinculação exclusiva a uma lacuna.' },
    { programId: 'par_novo_par', indicatorIds: [...new Set([...attendanceIds, ...integralIds, ...teacherIds, ...infrastructureIds, 'aee', 'eja_integrada_educacao_profissional_percentual', 'medio_tecnico_articulado_percentual'])], linkType: 'programmatic', evidence: 'O diagnóstico pode fundamentar iniciativas, mas atendimento depende de análise e orçamento.' },
    { programId: 'programa_nacional_infraestrutura_escolar', indicatorIds: infrastructureIds, linkType: 'conditioned', evidence: 'Há vínculo legal com infraestrutura, porém adesão, patamares e priorização dependem de ato do MEC.' },
    { programId: 'novo_pac_educacao', indicatorIds: ['creche', 'pre_escola', ...integralIds, 'salas_climatizadas', 'salas_acessiveis'], linkType: 'programmatic', evidence: 'Seleções podem apoiar expansão; requer janela, proposta e resultado oficial.' },
    { programId: 'escola_tempo_integral', indicatorIds: integralIds, linkType: 'direct', evidence: 'Fomenta criação, manutenção e qualificação de matrículas em tempo integral mediante pactuação.' },
    { programId: 'pnae', indicatorIds: [...attendanceIds, ...integralIds], linkType: 'programmatic', evidence: 'Alimentação apoia permanência e jornada, sem financiar diretamente a cobertura ou o desempenho.' },
    { programId: 'pnate', indicatorIds: ['creche', 'pre_escola', 'basico_6_17', 'basico_15_17', ...integralIds], linkType: 'programmatic', evidence: 'Apoia acesso e permanência de estudantes rurais elegíveis.' },
    { programId: 'caminho_escola', indicatorIds: ['creche', 'pre_escola', 'basico_6_17', 'basico_15_17', ...integralIds, 'salas_acessiveis'], linkType: 'programmatic', evidence: 'Veículos podem reduzir barreiras de transporte; aquisição depende da via e dos requisitos.' },
    { programId: 'pdde', indicatorIds: [...new Set([...infrastructureIds, 'conselho_escolar'])], linkType: 'programmatic', evidence: 'Recursos por escola/entidade podem apoiar pequenos investimentos e gestão; verificar ação e saldo.' },
    { programId: 'parfor', indicatorIds: ['adequacao_ai', 'adequacao_af', 'adequacao_em'], linkType: 'direct', evidence: 'Licenciatura na área de atuação pode atuar diretamente na adequação, condicionada ao edital.' },
    { programId: 'proeb_capes', indicatorIds: ['pos_graduacao'], linkType: 'direct', evidence: 'Mestrados profissionais e bolsas podem elevar a pós-graduação de docentes participantes.' },
    { programId: 'bolsa_mais_professores', indicatorIds: ['adequacao_ai', 'adequacao_af', 'adequacao_em', 'pos_graduacao', 'temporarios'], linkType: 'programmatic', evidence: 'Pode apoiar atração, permanência e formação apenas em áreas elegíveis.' },
    { programId: 'peate_rs', indicatorIds: ['creche', 'pre_escola', 'basico_6_17', 'basico_15_17'], linkType: 'programmatic', evidence: 'Apoia transporte de estudantes estaduais rurais em municípios habilitados.' },
    { programId: 'siope', indicatorIds: allIndicatorIds, linkType: 'no_proven_direct_link', evidence: 'Sistema de evidência orçamentária; não transfere recursos.' },
    { programId: 'siconfi_finbra', indicatorIds: allIndicatorIds, linkType: 'no_proven_direct_link', evidence: 'Sistema fiscal/contábil; não transfere recursos.' },
  ],
  matrixVersion: 'indicator-financing-links-v1',
  semantics: {
    conditioned: 'Vínculo dependente de condição, regulamentação ou critério oficial.',
    direct: 'O desenho atua diretamente no objeto, sem garantir elegibilidade ou efeito.',
    general_mde: 'Fonte geral de MDE; relação não exclusiva.',
    no_proven_direct_link: 'Não é fonte ou não há nexo direto comprovado.',
    programmatic: 'Pode apoiar intervenção relacionada, sem destinação automática ao indicador.',
  },
}

fs.writeFileSync(matrixOutputPath, `${JSON.stringify(matrix, null, 2)}\n`, 'utf8')
